...
<title>大家在说 - <?php echo IN_NAME; ?></title>
...
<ul class="tabs">
<li class="active"><a href="<?php echo rewrite_mode('user.php/feed/index/'); ?>"><span>大家在说</span></a></li>
<li><a href="<?php echo rewrite_mode('user.php/feed/friend/'); ?>"><span>好友在说</span></a></li>
<li><a href="<?php echo rewrite_mode('user.php/feed/me/'); ?>"><span>我的说说</span></a></li>
</ul>
</div>
<div id="content">
...
if($sort == 'asc'){
        $Arr = getuserpage("select * from ".tname('feed')." where in_type=0 order by in_addtime asc", 20, 4);
}else{
        $Arr = getuserpage("select * from ".tname('feed')." where in_type=0 order by in_addtime desc", 20, 3);
}
...
$del = $row['in_uid'] == $erduo_in_userid ? '&nbsp;<a href="javascript:void(0)" onclick="deldoing('.$row['in_id'].');" class="re gray">删除</a>' : '';
?>
<li>
...
<option value="<?php echo rewrite_mode('user.php/feed/index/'); ?>">时间倒序</option>
<option value="<?php echo rewrite_mode('user.php/feed/index/asc/'); ?>"<?php if($sort == 'asc'){echo " selected";} ?>>时间升序</option>
</select></li></ul>
</div></div>
</div>
<div id="bottom"></div>
