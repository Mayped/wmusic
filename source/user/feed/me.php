...
if($sort == 'asc'){
        $Arr = getuserpage("select * from ".tname('feed')." where in_uid=".$erduo_in_userid." and in_type=0 order by in_addtime asc", 20, 4);
}else{
        $Arr = getuserpage("select * from ".tname('feed')." where in_uid=".$erduo_in_userid." and in_type=0 order by in_addtime desc", 20, 3);
}
