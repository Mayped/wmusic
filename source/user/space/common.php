...
$uid = isset($space[3]) ? $space[3] : $space[1];
if(is_numeric($uid)){
	...
	if($row = $db->fetch_array($result)){
		...
		if($userlogined && $ear['in_userid'] !== $erduo_in_userid){
		        if($fid = $db->getone("select in_id from ".tname('footprint')." where in_uid=".$erduo_in_userid." and in_uids=".$ear['in_userid'])){
