...
<?php
$id = '';
$query = $db->query("select in_uid from ".tname('friend')." where in_uids=".$erduo_in_userid);
while ($rows = $db->fetch_array($query)) {
$friend = $db->getone("select in_id from ".tname('friend')." where in_uid=".$erduo_in_userid." and in_uids=".$rows['in_uid']);
if(!$friend){
$id .= $rows['in_uid'].',';
}
}
