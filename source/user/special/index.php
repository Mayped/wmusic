...
$Arr = getuserpage("select * from ".tname('special')." where in_uid=".$erduo_in_userid." and in_passed=0 and in_classid=".$cid." order by in_addtime desc", 10, 4);
}else{
$get = true;
$Arr = getuserpage("select * from ".tname('special')." where in_uid=".$erduo_in_userid." and in_passed=0 order by in_addtime desc", 10, 3);
}
?>
...
<title>已审专辑 - <?php echo IN_NAME; ?></title>
...
<ul class="tabs">
<li class="active"><a href="<?php echo rewrite_mode('user.php/special/index/'); ?>"><span>已审专辑</span></a></li>
...
<?php if($get){ ?>
<div class="c_mgs"><div class="ye_r_t"><div class="ye_l_t"><div class="ye_r_b"><div class="ye_l_b">以下是您制作且已通过审核的专辑列表</div></div></div></div></div>
<?php
$result = $db->query($Arr[2]);
$count = $db->num_rows($result);
if($count == 0){
?>
<div class="c_form">没有相关已审专辑。</div>
<?php }else{ ?>
...
<li style="margin: 5px 0 0;"><a href="<?php echo rewrite_mode('user.php/special/set/'.$row['in_id'].'/'); ?>" class="float_setspecial">淘歌</a></li>
</ul>
</div>
</li>
<?php } ?>
...
<?php
if(!IsNum($cid)){
        echo "<li class=\"current\"><a href=\"".rewrite_mode('user.php/special/index/')."\">不限分类</a></li>";
}else{
        echo "<li><a href=\"".rewrite_mode('user.php/special/index/')."\">不限分类</a></li>";
}
$res=$db->query("select * from ".tname('special_class')." order by in_id asc");
if($res){
        while ($rows = $db->fetch_array($res)){
                if($cid == $rows['in_id']){
                        echo "<li class=\"current\"><a href=\"".rewrite_mode('user.php/special/index/'.$rows['in_id'].'/')."\">".$rows['in_name']."</a></li>";
                }else{
                        echo "<li><a href=\"".rewrite_mode('user.php/special/index/'.$rows['in_id'].'/')."\">".$rows['in_name']."</a></li>";
                }
        }
}
?>
