...
if($message[3] !== 'user' && $message[3] !== 'admin'){
        $Arr = getuserpage("select * from ".tname('message')." where in_uids=".$erduo_in_userid." and in_isread=0 order by in_addtime desc", 10, 3);
}elseif($message[3] == 'user'){
        $Arr = getuserpage("select * from ".tname('message')." where in_uids=".$erduo_in_userid." and in_isread=0 and in_uid<>0 order by in_addtime desc", 10, 4);
