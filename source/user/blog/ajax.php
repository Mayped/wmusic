...
if($ac == 'share'){
	...
	if(!$userlogined){
		...
		if($fid = $db->getone("select in_id from ".tname('feed')." where in_icon='blog' and in_uid=".$erduo_in_userid." and in_tid=".$id)){
	...
	if(!$userlogined){
		echo 'return_1';
	}elseif($db->getone("select in_id from ".tname('blog_group')." where in_title='".$title."' and in_uid=".$erduo_in_userid)){
		echo 'return_2';
	}else{
		$setarr = array(
			'in_title' => $title,
	...
	if(!$userlogined){
		...
		echo 'return_3';
	}elseif($db->getone("select in_id from ".tname('blog_group')." where in_id<>".$gid." and in_title='".$title."' and in_uid=".$erduo_in_userid)){
		echo 'return_4';
	}else{
