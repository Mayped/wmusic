...
if(IsNum($bid)){
$get = $db->getrow("select * from ".tname('blog')." where in_uid=".$erduo_in_userid." and in_id=".$bid);
}else{
$get = false;
}
?>
...
<?php
$result=$db->query("select * from ".tname('blog_group')." where in_uid=".$erduo_in_userid." order by in_id desc");
if($result){
...
<tr><td><textarea class="userData" name="message" id="uchome-ttHtmlEditor" style="height:100%;width:100%;display:none;border:0px"><?php echo getblog($get['in_content']); ?></textarea><iframe src="<?php echo IN_PATH; ?>static/pack/editor/editor.php" name="uchome-ifrHtmlEditor" id="uchome-ifrHtmlEditor" scrolling="no" border="0" frameborder="0" style="width:100%;border: 1px solid #C5C5C5;" height="400"></iframe></td></tr>
<tr><td><input type="submit" class="submit" value="保存编辑" /></td></tr>
</table>
</form>
<?php }else{ ?>
<div id="content" style="width:100%;"><div class="c_form">日志不存在或您无权编辑。</div></div>
<?php } ?>
</div>
<div id="bottom"></div>
</div>
