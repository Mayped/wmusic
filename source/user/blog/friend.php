...
$Arr = getuserpage("select * from ".tname('blog')." where in_uid in ($id) order by in_addtime desc", 20, 3);
...
<title>好友的日志 - <?php echo IN_NAME; ?></title>
...
<li class="active"><a href="<?php echo rewrite_mode('user.php/blog/friend/'); ?>"><span>好友的日志</span></a></li>
...
Jquery('.entry').imagesLoaded(function() {
	...
	Jquery(window).scroll(function() {
		...
		if ((c + b + 150 >= a) && istrue && p > 1 && page < p) {
			istrue = false;
			page++;
			Jquery.get("<?php echo rewrite_mode('user.php/blog/friend/p" + page + "/'); ?>?" + new Date().getTime(),
			function(result) {
				if (result) {
