...
$Arr = getuserpage("select * from ".tname('photo')." where in_uid in ($id) order by in_addtime desc", 20, 3);
...
<title>好友的照片 - <?php echo IN_NAME; ?></title>
...
<li class="active"><a href="<?php echo rewrite_mode('user.php/photo/friend/'); ?>"><span>好友的照片</span></a></li>
...
Jquery('.list').imagesLoaded(function() {
	...
	Jquery(window).scroll(function() {
		...
		if ((c + b + 150 >= a) && istrue && p > 1 && page < p) {
			istrue = false;
			page++;
			Jquery.get("<?php echo rewrite_mode('user.php/photo/friend/p" + page + "/'); ?>?" + new Date().getTime(),
			function(result) {
				if (result) {
