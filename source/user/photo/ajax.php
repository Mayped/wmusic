...
if($ac == 'share'){
	...
	if(!$userlogined){
		...
		$nums = $db->num_rows($db->query("select * from ".tname('photo')." where in_gid=".$id));
		if($fid = $db->getone("select in_id from ".tname('feed')." where in_icon='photo' and in_uid=".$erduo_in_userid." and in_tid=".$id)){
	...
	if(!$userlogined){
		echo 'return_1';
	}elseif($db->getone("select in_id from ".tname('photo_group')." where in_title='".$photo."' and in_uid=".$erduo_in_userid)){
		echo 'return_2';
	}else{
		$setarr = array(
			'in_pid' => 0,
	...
	if(!$userlogined){
		...
		echo 'return_3';
	}elseif($db->getone("select in_id from ".tname('photo_group')." where in_id<>".$gid." and in_title='".$title."' and in_uid=".$erduo_in_userid)){
		echo 'return_4';
	}else{
	...
	if(!$userlogined){
		...
		echo 'return_2';
	}elseif($row['in_uid'] !== $erduo_in_userid){
		echo 'return_3';
	}else{
	...
	if(!$userlogined){
		...
		echo 'return_3';
	}elseif($puid !== $erduo_in_userid){
		echo 'return_4';
	}elseif($guid !== $erduo_in_userid){
		echo 'return_5';
	}else{
