...
var listenMsg = {
        ...
        mid:0,
        gid:0,
        queryUrl:"http://" + in_server + "/api/chat.php",
        player:'<embed style="position:absolute;top:-100000px" width="0" height="0" type="application/x-shockwave-flash" swliveconnect="true" allowscriptaccess="sameDomain" menu="false" flashvars="sFile=' + in_path + 'source/plugin/chat/api/msg.mp3" src="' + in_path + 'source/plugin/chat/api/sound.swf" />',
        stop:function() {
        ...
        start:function(_uid, _type, _do) {
                ajax({
                        type:"get",
                        url:listenMsg.queryUrl + "?ac=start&type=" + _type + "&do=" + _do + "&uid=" + _uid,
                        dataType:"jsonp",
        ...
        group:function(_num) {
                ajax({
                        type:"get",
                        url:listenMsg.queryUrl + "?ac=group&num=" + _num,
                        dataType:"jsonp",
                        success:function(data) {
                                if (data["group"] != -1) {
                                        clearTimeout(listenMsg.gid);
        ...
        login:function() {
                ...
                ajax({
                        type:"get",
                        url:listenMsg.queryUrl + in_info,
                        dataType:"jsonp",
                        success:function(data) {
                                if (data["uid"] == -1) {
                                        ...
                                        $(".chat01_title span").attr("uid", data["uid"]);
                                        $(".chat01_title span").text(data["password"]);
                                        $(".talkTo a").text("公共频道");
                                        $(".talkTo a").attr("uid", 0);
        ...
        list:function(_uid) {
                ajax({
                        type:"get",
                        url:listenMsg.queryUrl + "?ac=list&uid=" + _uid,
                        dataType:"jsonp",
                        success:function(data) {
        ...
        send:function() {
                ...
                ajax({
                        type:"get",
                        url:listenMsg.queryUrl + "?ac=send&text=" + escape($("#textarea").val()) + "&uname=" + $(".talkTo a").text() + "&uid=" + $(".talkTo a").attr("uid"),
                        dataType:"jsonp",
                        success:function(data) {
                                if (data["send"] == -1) {
        ...
        lib = {
                ...
                avatar:function() {
                        $.layer({
                                ...
                                area:[ "auto", "auto" ],
                                page:{
                                        html:'<embed src="http://' + in_server + "/static/pack/upload/camera.swf" + in_avatar + $(".chat01_title span").attr("uid") + ":" + $(".chat01_title span").text() + '" width="459" height="266" wmode="transparent" allowScriptAccess="always" type="application/x-shockwave-flash"></embed>'
                                }
                        });
                },
