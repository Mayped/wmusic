...
if($userlogined){
        if(!$db->getone("select in_userid from ".tname('user')." where in_userid=".$erduo_in_userid." and DATEDIFF(DATE(in_logintime),'".date('Y-m-d')."')=0")){
	        $db->query("update ".tname('user')." set in_points=in_points+".IN_LOGINDAYPOINTS.",in_rank=in_rank+".IN_LOGINDAYRANK.",in_logintime='".date('Y-m-d H:i:s')."' where in_userid=".$erduo_in_userid);
	        $setarr = array(
		        'in_uid' => 0,
		        'in_uname' => '系统用户',
        ...
        if($erduo_in_grade == 1){
	        $vipenddate = strtotime($erduo_in_vipenddate)-time();
	        if($vipenddate <= 0){
		        $db->query("update ".tname('user')." set in_grade=0,in_vipgrade=0,in_vipindate='0000-00-00 00:00:00',in_vipenddate='0000-00-00 00:00:00' where in_userid=".$erduo_in_userid);
		        $setarrs = array(
			        'in_uid' => 0,
			        'in_uname' => '系统用户',
			        'in_uids' => $erduo_in_userid,
