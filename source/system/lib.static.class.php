...
class Lib{
	...
	public static function P_age($sqlstr, $pagesok, $li, $lis, $lisp, $pagenum, $pathinfo, $pages, $self){
	        ...
	        $url = explode('/', $self);
 	        ...
 	        for($k=1;$k<=$pagejs;$k++){
		        if($k == $pages){
			        $pagelist .= '<option selected="selected">第'.$k.'页</option>';
	                }else{
			        $pagelist .= '<option value="'.rewrite_mode(!empty($url[$pathinfo]) ? str_replace($url[$pathinfo], 'p'.$k, $self) : $self.'p'.$k.'/', true).'">第'.$k.'页</option>';
	                }
 	        }
 	        $pagelist .= '</select>';
 	        if($pagejs <= $pagenum){
