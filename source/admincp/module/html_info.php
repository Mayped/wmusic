...
if($action == 'class'){
        ...
        if($classid == 0){
                $info = $db->query("select * from ".tname($table));
                ...
                $info = $db->query("select * from ".tname($table)." where in_classid=".intval($classid));
                $num = $db->num_rows($info);
                $size = $num > IN_HTMLSIZE ? ceil($num/IN_HTMLSIZE) : 1;
                if($size > 1){
        ...
        if($dayid == 0){
                $info = $db->query("select * from ".tname($table)." where DATEDIFF(DATE(in_addtime),'".date('Y-m-d')."')=0");
                $num = $db->num_rows($info);
                $size = $num > IN_HTMLSIZE ? ceil($num/IN_HTMLSIZE) : 1;
                if($size > 1){
                ...
                $info = $db->query("select * from ".tname($table)." where DATEDIFF(DATE(in_addtime),'".date('Y-m-d')."')=-".$dayid);
                $num = $db->num_rows($info);
                $size = $num > IN_HTMLSIZE ? ceil($num/IN_HTMLSIZE) : 1;
                if($size > 1){
