...
function EditBoard($Arr,$url){
	global $db;
	$music=$db->num_rows($db->query("select * from ".tname('music')." where in_uid=".$Arr[0]));
	$special=$db->num_rows($db->query("select * from ".tname('special')." where in_uid=".$Arr[0]));
	$singer=$db->num_rows($db->query("select * from ".tname('singer')." where in_uid=".$Arr[0]));
	$video=$db->num_rows($db->query("select * from ".tname('video')." where in_uid=".$Arr[0]));
	$in_username = $Arr[1];
	$in_mail = $Arr[2];
	$in_ismail = $Arr[3];
	$in_sex = $Arr[4];
...
<tr><td colspan="2" class="td27">邮箱:</td></tr>
