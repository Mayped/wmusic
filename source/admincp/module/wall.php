...
function all_save(form){
		...
		layer.tips('删除留言不可逆，请谨慎操作！', '#chkall', {
			tips: 3
		});
        }
}
...
switch($action){
	...
	case 'keyword':
		...
		main("select * from ".tname('wall')." where in_content like '%".$key."%' or in_uname like '%".$key."%' order by in_addtime desc",20);
		break;
	default:
		main("select * from ".tname('wall')." order by in_addtime desc",20);
		break;
	}
?>
</body>
...
?>
<div class="container">
<script type="text/javascript">parent.document.title = 'Ear Music Board 管理中心 - 用户管理 - 所有留言';if(parent.$('admincpnav')) parent.$('admincpnav').innerHTML='用户管理&nbsp;&raquo;&nbsp;所有留言';</script>
<div class="floattop"><div class="itemtitle"><h3>所有留言</h3></div></div><div class="floattopempty"></div>
<table class="tb tb2">
<tr><th class="partition">技巧提示</th></tr>
<tr><td class="tipsblock"><ul>
<li>可以输入留言内容、所属会员等关键词进行搜索</li>
</ul></td></tr>
</table>
<table class="tb tb2">
...
<td><a href="<?php echo getlink($row['in_uids']); ?>" target="_blank" class="act"><?php echo $content; ?></a></td>
...
<?php
	...
	function alldel(){
		...
		if($in_id==0){
			...
			$db->query("delete from ".tname('wall')." where in_id in ($in_id)");
