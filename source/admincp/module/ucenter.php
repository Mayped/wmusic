...
<li class="lightnum">您也可以登录 UCenter 用户管理中心自定义安装一条应用，然后复制该应用的配置信息粘贴到 ./client/ucenter.php 文件来完成通信设置，连接后将与 UCenter 进行会员数据的同步。</li>
</ul></td></tr>
</table>
<?php if($type==1){ ?>
...
<tr><td colspan="2" class="td27">UCenter 的 URL 地址:</td></tr>
...
<tr><td colspan="2" class="td27">UCenter 的创始人密码:</td></tr>
...
function ucsubmit(){
	...
	if($status != 'UC_STATUS_OK') {
		echo '<div class="container"><div class="floattop"><div class="itemtitle"><h3>UCenter</h3></div></div><div class="floattopempty"></div>';
		...
		echo '</ul></td></tr>';
		echo '</table>';
		echo '<h3>Ear Music 提示</h3>';
		echo '<div class="infobox"><br /><h4 class="marginbot normal" style="color:#C00">'.$status.'</h4><br />';
		echo '<form method="post" action="?iframe=ucenter&action=ucsubmit"><table class="tb tb2"><tr><td class="td29">';
	...
	$postdata = "m=app&a=add&ucfounder=&ucfounderpw=".urlencode($ucfounderpw)."&apptype=".urlencode('OTHER')."&appname=".urlencode('Ear Music')."&appurl=".urlencode($app_url)."&appip=&appcharset=".IN_CHARSET.'&appdbcharset='.IN_DBCHARSET.'&release='.UC_RELEASE;
