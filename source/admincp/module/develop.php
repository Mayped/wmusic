...
<body>
<div class="container">
<script type="text/javascript">parent.document.title = 'Ear Music Board 管理中心 - 安装应用';if(parent.$('admincpnav')) parent.$('admincpnav').innerHTML='安装应用';</script>
...
function install_ation($type){
	...
	global $db;
        if($type==1){
                ...
                if($one=$db->getone("select in_id from ".tname('template')." where in_path='template/".$dir."/".$html."/'")){
                        $db->query("update ".tname('template')." set in_name='".$name."',in_default=0,in_addtime='".date('Y-m-d H:i:s')."' where in_id=".$one);
                }else{
