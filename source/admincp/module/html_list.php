...
if($listid == 0){
        $list = $db->query("select * from ".tname($table));
        $size = $db->num_rows($list);
        if($size > 1){
...
while($row = $db->fetch_array($list)){
        $count = $db->num_rows($db->query("select * from ".tname($info)." where in_passed=0 and in_classid=".$row['in_id']));
...
while($rows = $db->fetch_array($class)){
        $counts = $db->num_rows($db->query("select * from ".tname($info)." where in_passed=0 and in_classid=".$rows['in_id']));
